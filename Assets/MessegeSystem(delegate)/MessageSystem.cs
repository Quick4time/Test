...
// Оптимизация Страница 77.
public delegate bool MessageHandlerDelegate(BaseMessage messege);
...
public class MessagingSystem : SingletonAsComponent<MessagingSystem> {
    ...
    public bool AttachListener(System.Type type, MessageHandlerDelegate handler)
    {
        if (type == null)
        {
            Debug.Log("MessagingSystem: AttachListener failded due to no message type specified");
            return false;
        }
